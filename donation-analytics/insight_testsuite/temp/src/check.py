...
def process(line):
    ...
    if other_id or not amount.isnumeric() or not date.isnumeric() or not recepient or len(date) != 4 or amount == '' or int(amount) == 0 or not name: #check isnumeric vs isdigit vs isdecimal
        pass
    else:
...
with open('../input/easy_test_itcont.txt') as f:
    for line in f:
